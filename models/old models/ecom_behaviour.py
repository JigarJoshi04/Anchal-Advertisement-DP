...
def getOutput():
    ...
    visUsrData["VisitDateTime"] = visUsrData["VisitDateTime"].progress_apply(
        correctTimeStamp
    )
    ...
    visUsrData["VisitDateTime"] = visUsrData.groupby(["UserID", "webClientID"])[
        "VisitDateTime"
    ].transform(lambda x: x.fillna(x.min()))
    visUsrData["Activity"] = (
        visUsrData.sort_values(["UserID", "VisitDateTime"])
        .groupby("UserID")["Activity"]
    ...
    visUsrData["ProductID"] = visUsrData["ProductID"].apply(
        lambda x: x if x != x else str(x).lower()
    )
    ...
    visUsrData.loc[
        (visUsrData["isActive"] == 1) & (visUsrData["SevenDays"] == 1), "is7Active"
    ] = 1

    visUsrData["VisitDate"] = visUsrData["VisitDateTime"].dt.date
    visUsrData["Activity"] = visUsrData["Activity"].apply(
        lambda x: x if x != x else str(x).lower()
    )

    visUsrData["Pageloads_last_7_days"] = 0
    visUsrData.loc[
        (visUsrData["Activity"] == "pageload") & (visUsrData["SevenDays"] == 1),
        "Pageloads_last_7_days",
    ] = 1

    visUsrData["Clicks_last_7_days"] = 0
    visUsrData.loc[
        (visUsrData["Activity"] == "click") & (visUsrData["SevenDays"] == 1),
        "Clicks_last_7_days",
    ] = 1

    visUsrData["FifteenDaysActive"] = 0
    visUsrData.loc[
        ((visUsrData["FifteenDays"] == 1) & (visUsrData["isActive"] == True)),
        "FifteenDaysActive",
    ] = 1
    visUsrData["pageloads_actvity"] = 0
    visUsrData.loc[(visUsrData["Activity"] == "pageload"), "pageloads_actvity"] = 1

    visUsrData["ProductID"] = (
        visUsrData.sort_values(["UserID", "VisitDateTime"])
    ...
    df = pd.DataFrame(
        visUsrData.groupby(["UserID"])["webClientID"]
        .count()
        .reset_index()
        .drop("webClientID", axis=1)
    )
    df
    ...
    df = df.merge(
        visUsrData.groupby(["UserID", "VisitDate", "is7Active"])
        .count()
        .reset_index()
        .groupby("UserID")["is7Active"]
        .sum()
        .reset_index(),
        on="UserID",
        how="left",
    ...
    visUsrData["Signup Date"] = pd.to_datetime(
        visUsrData["Signup Date"], format="%Y-%m-%d %H:%M:%S"
    )
    visUsrData["Signup Date"] = visUsrData["Signup Date"].dt.tz_localize(None)
    visUsrData["User_Vintage"] = (
        visUsrData["VisitDateTime"].max() - visUsrData["Signup Date"]
    ).dt.days
    ...
    df = df.merge(
        visUsrData.groupby(["UserID"])["User_Vintage"].max().reset_index(),
        on="UserID",
        how="left",
    )
    ...
    df["Most_Active_OS"] = (
        visUsrData.groupby(["UserID"])["OS"].agg(lambda x: stats.mode(x)[0][0]).values
    )
    ...
    df["Pageloads_last_7_days"] = (
        visUsrData.groupby(["UserID"])["Pageloads_last_7_days"].sum().values
    )
    df["Clicks_last_7_days"] = (
        visUsrData.groupby(["UserID"])["Clicks_last_7_days"].sum().values
    )
    ...
    df = df.merge(
        visUsrData.sort_values(["UserID", "VisitDateTime"])[mask]
        .groupby(["UserID"])
        .agg({"ProductID": "last"})
        .reset_index()
        .rename(columns={"ProductID": "Recently_Viewed_Product"}),
        on="UserID",
        how="left",
    )
    ...
    df = df.merge(
        visUsrData.sort_values(["UserID", "VisitDateTime"], ascending=[True, False])[
            mask2
        ]
        .groupby(["UserID"])
        .agg({"ProductID": lambda x: stats.mode(x)[0][0]})
        .reset_index()
        .rename(columns={"ProductID": "Most_Viewed_product_15_Days"}),
        on="UserID",
        how="left",
    )
    ...
    visUsrData["ProductID"] = visUsrData.groupby(["UserID"])["ProductID"].transform(
    ...
    df = df.merge(
        ...
        .reset_index()
        .rename(columns={"ProductID": "No_Of_Products_Viewed_15_Days"}),
        on="UserID",
        how="left",
    )
    ...
    df.to_csv(
        dir + "\\datasets\\output\\ecom_behavior_input_feats_v27.csv", index=False
    )
